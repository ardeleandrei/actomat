...
def preprocess_image_variant2(raw_bytes, max_dim=1600, min_dim_for_upscale=900):
    ...
    # Optional: Try *not* upscaling, or use a different upscale factor
    if min(w, h) < min_dim_for_upscale:
        scale = 3  # Try a bigger upscale for very small images
        image = image.resize((w*scale, h*scale), Image.LANCZOS)
        w, h = image.size
    ...
    # Convert to grayscale and CLAHE or stronger contrast
    image = image.convert("L")
    image = ImageOps.autocontrast(image)
    image = image.filter(ImageFilter.SHARPEN)
    # Or: more filter, or even multiple sharpens: image = image.filter(ImageFilter.SHARPEN).filter(ImageFilter.SHARPEN)
...
def preprocess_image_variant3(raw_bytes, max_dim=1600, min_dim_for_upscale=900):
    ...
    # Upscale agresiv dacă e foarte mică
    if min(w, h) < min_dim_for_upscale:
        scale = 4  # chiar mai agresiv
        image = image.resize((w*scale, h*scale), Image.LANCZOS)
        w, h = image.size
    ...
    # Convertim la grayscale, creștem contrastul și aplicăm sharpen agresiv + denoise
    image = image.convert("L")
    image = ImageOps.autocontrast(image)
...
try:
    ...
    if torch.cuda.is_available():
        ...
        gpu_info = f"CUDA device detected: {gpu_name}"
    else:
        gpu_info = "No CUDA device detected via torch."
except ImportError:
    gpu_info = "PyTorch not installed; can't check for CUDA device. Assuming llama.cpp will use what it's compiled for."

print(f"[INIT] Llama.cpp n_gpu_layers = -1 (all possible layers on GPU if available). {gpu_info}")
...
with suppress_stdout_stderr():
    llm = Llama(
        ...
        n_ctx=2048,
        n_threads=n_threads,
        n_gpu_layers=0 if not torch.cuda.is_available() else -1,  # Use all layers on GPU if available
    )
    import llama_cpp
...
async def extract(file: UploadFile = File(...)):
    ...
    # === Preprocess & OCR: run both variants ===
    preprocessed1 = preprocess_image(raw, max_dim=1600)
    preprocessed2 = preprocess_image_variant2(raw, max_dim=1600)
    preprocessed3 = preprocess_image_variant3(raw, max_dim=1600)

    print("[API] Image preprocessed (2 variants).")

    import numpy as np  # needed for PaddleOCR
    ocr_results = []
    ...
    # === LLM PROMPT: Compare both OCRs and extract best fields ===
    system_prompt = (
    ...
    print("[API] Sending prompt to LLM (compare both OCR variants)...")
    ...
    # === LLM PROMPT 2 (DOAR DACA E NEVOIE) ===
    required_fields = [
